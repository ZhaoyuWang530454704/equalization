...
//        CoreProcess query = new CoreProcess("D:");
        CoreProcess query = new CoreProcess("C:\\Users\\Chloe\\Desktop\\HLP汉语言处理\\");
        String[] questionArr = new String[] {"人民银行牵头负责什么基本公共服务项目"};
        for(String que: questionArr){

            ArrayList<String> question = query.analyQuery(que);
            System.err.println(question.get(0)+question.get(1));
        }
    }
    ...
    public void TestRDD(){
        ...
        System.out.println(nb_model.predict(vTest));// 分类结果 == 返回分类的标签值
        /**
         * 计算测试目标向量与训练样本数据集里面对应的各个分类标签匹配的概率结果
         */
        System.out.println("test"+nb_model.predictProbabilities(vTest));
